(() => {
    ...
    function slider() {
        ...
        if (discountSliders.length) discountSliders.forEach((slider => {
            new Swiper(slider, {
                speed: 1e3,
        ...
        if (discountSlidersProducts.length && window.matchMedia("(max-width: 959px)").matches) discountSlidersProducts.forEach((slider => {
            new Swiper(slider, {
                speed: 1e3,
                modules: [ Autoplay, Navigation ],
                slidesPerView: 2,
                ...
                navigation: {
                    prevEl: slider.closest(".discount__slider-wrapper").querySelector(".discount__slider-products-btn._prev"),
                    nextEl: slider.closest(".discount__slider-wrapper").querySelector(".discount__slider-products-btn._next")
                }
            });
        }));
        const singleSldier = document.querySelector(".single__slider");
        if (singleSldier && window.matchMedia("(max-width:943px)").matches) {
            new Swiper(singleSldier, {
                speed: 1e3,
                modules: [ Autoplay, Pagination ],
                slidesPerView: 1,
                spaceBetween: 8,
    ...
    function search() {
        ...
        if (searchMenu) {
            ...
            input.addEventListener("input", (e => {
                if (e.target.value) searchClear.classList.add("active"); else searchClear.classList.remove("active");
            }));
            ...
            searchBtn.addEventListener("click", (e => {
                e.stopPropagation();
                if (searchMenu.classList.contains("open")) closeSearch(); else openSearch();
            }));
            function openSearch() {
                closeBasket();
                burgerClose();
    ...
    function basket() {
        ...
        if (basket) {
            basketBtn.addEventListener("click", (e => {
                e.stopPropagation();
                if (basket.classList.contains("open")) closeBasket(); else openBasket();
            }));
    ...
    function burger() {
        ...
        burgerOpen.addEventListener("click", (e => {
            e.stopPropagation();
            if (burger.classList.contains("open")) burgerClose(); else {
                closeBasket();
                closeSearch();
    ...
    function productLike() {
        ...
        if (buttons.length) buttons.forEach((b => {
    ...
    function productCounter() {
        ...
        if (counters.length) {
            ...
            countersMinus.forEach((counter => {
                ...
                counter.addEventListener("click", (() => {
                    if (+counterNumber.textContent > 0) counterNumber.textContent = +counterNumber.textContent - 1;
                }));
            }));
        }
    }
    function cookie() {
        ...
        if (cookieHtml) {
            if (!document.cookie.includes("cookie=on")) cookieHtml.classList.add("active");
            btnOn.addEventListener("click", (() => {
                const date = Date.now() + 60 * 60 * 1e3;
    ...
    function fancy() {
        Oe.bind('[data-fancybox="product-gallery"]', {});
    }
    function copy() {
        ...
        if (items.length && window.matchMedia("(min-width: 1139px)").matches) {
            ...
            function handlerScroll(index) {
                if (index <= items.length - 1) {
                    ...
                    if (window.scrollY >= top && window.scrollY <= top + items[index].clientHeight) {
                        anchorsItems.forEach((a => a.classList.remove("active")));
                        anchorsItems[index].classList.add("active");
                    } else handlerScroll(index + 1);
                }
            }
        }
    }
